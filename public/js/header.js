$(function() {
    ...
    var dialogSetUp = (function() {
        function checkToSeeIfOnCreatePage(){
        ...
        function authButtonClicked() {
            $("#signup, #login, #skip").on("click", function(e) {
                ...
                var dataToSend
                    if (checkToSeeIfOnCreatePage()  && reviewDataValidation() ) {
                        ...
                        // isSubmit = false;
                    }else if(!checkToSeeIfOnCreatePage()){
                        //means not logging in at `create` page. they click signin at header
                        dataToSend = getDataForAuth(optionChosen);
                        dataToSend = JSON.stringify(dataToSend);
                        sendDataToServer(dataToSend);
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if(!isObjectEmpty(data)){
                                ...
                                $("#authFeedback").remove();
                                if(optionChosen === "signup"){
                                    $(".signup").prepend("<div id = 'authFeedback' class = 'text-danger'>"+ data.info +"</div>");
                                }else{
                                    $(".login").prepend("<div id = 'authFeedback' class = 'text-danger'>"+ data.info +"</div>");
                                }

                            }else{
                                $('.modal--close').trigger('click');
                                window.location.replace("/reviewfor/" + window.location.pathname.split("/")[2] );
                            }
        ...
        function reviewDataValidation() {
            ...
            if (!$overall.data('rated')) {
                //possible change to scroll to top of element container
            ...
            if (!$title.val()) {
                ...
                $('html, body').animate({ scrollTop: $title.offset().top - $title.height() - 200 }, 500, 'swing');
                event.preventDefault();
                $title.focus();
            ...
            if (!$reviewText.val()) {
                ...
                $('html, body').animate({ scrollTop: $reviewText.offset().top - $reviewText.height() - 200 }, 500, 'swing');
                event.preventDefault();
                $reviewText.focus();
                $('.header.create').addClass('hidden');
                return false;
        ...
        function getDataForReview() {
            ...
            data.photo = [];
            data.companyName = window.location.pathname.split("/")[2];
            if(data)
